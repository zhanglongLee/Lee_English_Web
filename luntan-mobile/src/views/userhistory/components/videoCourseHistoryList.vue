...
import { getUserVideoCourseHistory, deleteUserVideoCourseHistory } from "@/api/videoCourse";
...
export default {
  ...
  methods: {
    getList() {
      ...
      getUserVideoCourseHistory(data)
        .then((res) => {
          this.loading = false;
          const rows = res.data;
    ...
    async beforeClose({ name, position, instance }) {
      switch (position) {
        ...
        case "outside":
          ...
          break;
        case "right":
          await deleteUserVideoCourseHistory(this.videoCourses[name].history_id);
          Toast.success("删除视频课程历史记录成功！");
