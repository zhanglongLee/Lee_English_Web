<template>
  <div class="hbl-fa">
    <div class="hbl-comm" v-if="isLogin">
      ...
      <div class="comment" :style="{ width: commentWidth }">
        ...
        <div v-if="buttonMap[0]" class="hbl-owo">
          ...
          <div class="publish publish-btn">
            <button class="btn" @click="pSend()">发送</button>
            <button class="btn btn-cancel" @click="cancel(0)">取消</button>
          </div>
        </div>
      </div>
    </div>
    <div class="comment-mask" v-if="!isLogin">登录才可以参与评论~</div>
    <div class="comm">
    ...
    <div v-for="(item, index) in showComment" :key="index" class="hbl-child">
      <div class="reply"></div>
      <div class="content">
        <div class="comment-f">
          <avatar
        ...
        <div class="comment-f">
          <div>
            <div class="nickname author" v-if="item.commentUser.nickName">
        ...
        <div class="reply-content reply-fa">
          <div class="reply-font" @click="doReply(item.id)">
            <div v-if="isLogin&&item.commentUser.id !== currentUserId">
          ...
          >
            ...
            <div v-if="buttonMap[item.id]" class="hbl-owo">
              ...
              <div class="publish publish-btn">
                <button
                  class="btn"
                  @click="cSend(item.id, item.commentUser.id, item.id)"
                >
                  发送
                </button>
      ...
      <div v-for="(ritem, jndex) in item.childrenList" :key="jndex" class="children">
        ...
        <div class="content">
          ...
          <div class="reply-content">
            <div class="cc cc-to" v-if="ritem.targetUser.nickName">
          ...
          <div class="reply-content reply-fa">
            <div class="reply-font" @click="doReply(ritem.id)">
              <div v-if="isLogin&&ritem.commentUser.id !== currentUserId">
                <img src="./img/icon/reply.png" class="icon-reply" /><font
                  class="icon-reply icon-hf"
                >回复</font>
              </div>
...
export default {
  ...
  watch: {
    ...
    // '$route':'routeChange'
    commentList(){
      this.showComment = this.commentList
      this.currentUserId = this.$store.state.userInfo.id || 0 
    },
    '$store.state.token':{
